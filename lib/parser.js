...
class RouteParser {
  constructor() {
    ...
    this.routerMountMap = new Map();
  }

  /**
   * First-pass scan of an entry file to build the router mount map.
   * Call this on your app.js / index.js BEFORE parsing route files.
   * @param {string} filePath - Path to the entry file
   */
  scanMountPoints(filePath) {
    try {
      ...
      // Build a local var → import-source map for this file
      const localImports = {}; // varName → resolved absolute path
      ...
      traverse(ast, {
        VariableDeclarator: (nodePath) => {
          ...
          if (
            ...
            const resolved = this.resolveImportPath(requirePath, filePath);
            if (resolved && node.id.type === 'Identifier') {
              localImports[node.id.name] = resolved;
            }
          }
        },
        ImportDeclaration: (nodePath) => {
          const { node } = nodePath;
          const resolved = this.resolveImportPath(node.source.value, filePath);
          if (resolved) {
            node.specifiers.forEach(spec => {
              if (spec.type === 'ImportDefaultSpecifier') {
                localImports[spec.local.name] = resolved;
              }
            });
          }
        }
      ...
      // Now find all app.use('/some-path', routerVar) calls
      traverse(ast, {
        CallExpression: (nodePath) => {
          ...
          if (
            ...
            const mountPath = node.arguments[0].value;

            // Support: app.use('/path', router)
            //      and app.use('/path', mw1, mw2, router)
            for (let i = 1; i < node.arguments.length; i++) {
              const arg = node.arguments[i];
              if (arg.type === 'Identifier' && localImports[arg.name]) {
                this.routerMountMap.set(localImports[arg.name], mountPath);
              }
            }
          }
        }
      ...
      console.warn(`Warning: Could not scan mount points in ${filePath}: ${err.message}`);
    }
  }
  ...
  parseFile(filePath, inferredBasePath = '') {
    try {
      ...
      let basePath =
        this.routerMountMap.get(filePath) ||
        inferredBasePath ||
        '';
      if (!basePath) {
